...
export async function GET(
  ...
  if (!normalizedOutTradeNo) {
    return billingJson({ ok: false, error: { code: "billing_order_missing", message: "Order is missing." } }, 400);
  }

  try {
    return billingJson(
      await requestSiteAuthJson(`/v1/billing/membership/orders/${encodeURIComponent(normalizedOutTradeNo)}`, {
