...
export async function generateMetadata({
  ...
  return buildSeoResourceMetadata("opcOperators", locale);
}

export default async function OpcContentOpsSystemPage({
  searchParams,
}: {
  searchParams: SeoSearchParams;
}) {
  const locale = await resolveSeoLocale(searchParams);
  return <SeoResourcePage kind="opcOperators" locale={locale} />;
}
