...
export async function generateMetadata({ searchParams }: { searchParams: SeoSearchParams }): Promise<Metadata> {
  ...
  return buildSeoResourceMetadata("fdeAiDeliveryChecklist", locale);
}

export default async function FdeAiDeliveryChecklistPage({ searchParams }: { searchParams: SeoSearchParams }) {
  const locale = await resolveSeoLocale(searchParams);
  return <SeoResourcePage kind="fdeAiDeliveryChecklist" locale={locale} />;
}
