...
export async function generateMetadata({
  ...
  return buildSeoResourceMetadata("founderDecisionWorkflow", locale);
}

export default async function FounderAiDecisionWorkflowPage({
  searchParams,
}: {
  searchParams: SeoSearchParams;
}) {
  const locale = await resolveSeoLocale(searchParams);
  return <SeoResourcePage kind="founderDecisionWorkflow" locale={locale} />;
}
