...
export async function generateMetadata({
  ...
  return buildSeoResourceMetadata("indieAgentWorkbench", locale);
}

export default async function IndieAgentWorkbenchPage({
  searchParams,
}: {
  searchParams: SeoSearchParams;
}) {
  const locale = await resolveSeoLocale(searchParams);
  return <SeoResourcePage kind="indieAgentWorkbench" locale={locale} />;
}
