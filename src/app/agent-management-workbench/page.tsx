...
export async function generateMetadata({
  ...
  return buildSeoResourceMetadata("agentManagementWorkbench", locale);
}

export default async function AgentManagementWorkbenchPage({
  searchParams,
}: {
  searchParams: SeoSearchParams;
}) {
  const locale = await resolveSeoLocale(searchParams);
  return <SeoResourcePage kind="agentManagementWorkbench" locale={locale} />;
}
