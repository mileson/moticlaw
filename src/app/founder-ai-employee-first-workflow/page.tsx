...
export async function generateMetadata({
  ...
  return buildSeoResourceMetadata("founderFirstWorkflow", locale);
}

export default async function FounderAiEmployeeFirstWorkflowPage({
  searchParams,
}: {
  searchParams: SeoSearchParams;
}) {
  const locale = await resolveSeoLocale(searchParams);
  return <SeoResourcePage kind="founderFirstWorkflow" locale={locale} />;
}
