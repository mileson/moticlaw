...
export async function generateMetadata({
  ...
  return buildSeoResourceMetadata("localDeployment", locale);
}

export default async function LocalDeploymentPage({
  searchParams,
}: {
  searchParams: SeoSearchParams;
}) {
  const locale = await resolveSeoLocale(searchParams);
  return <SeoResourcePage kind="localDeployment" locale={locale} />;
}
