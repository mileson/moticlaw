...
export async function generateMetadata({
  ...
  return buildSeoResourceMetadata("founders", locale);
}

export default async function AiPartnerForFoundersPage({
  searchParams,
}: {
  searchParams: SeoSearchParams;
}) {
  const locale = await resolveSeoLocale(searchParams);
  return <SeoResourcePage kind="founders" locale={locale} />;
}
