...
export async function generateMetadata({
  ...
  return buildSeoResourceMetadata("localAiAgentPlatformComparison", locale);
}

export default async function LocalAiAgentPlatformVsCloudPage({
  searchParams,
}: {
  searchParams: SeoSearchParams;
}) {
  const locale = await resolveSeoLocale(searchParams);
  return <SeoResourcePage kind="localAiAgentPlatformComparison" locale={locale} />;
}
