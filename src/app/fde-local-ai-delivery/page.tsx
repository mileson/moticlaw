...
export async function generateMetadata({
  ...
  return buildSeoResourceMetadata("fdeLocalAiDelivery", locale);
}

export default async function FdeLocalAiDeliveryPage({
  searchParams,
}: {
  searchParams: SeoSearchParams;
}) {
  const locale = await resolveSeoLocale(searchParams);
  return <SeoResourcePage kind="fdeLocalAiDelivery" locale={locale} />;
}
